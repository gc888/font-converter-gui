...
public class FontSubEventHandler implements EventHandler<ActionEvent> {
    ...
    public FontSubEventHandler(TextField directory, TextArea subsetTxtArea, CheckBox stripHintCb, ToggleGroup formatGroup) {
                ...
                public FileVisitResult visitFile(java.nio.file.Path file, BasicFileAttributes attrs) throws IOException {
                    ...
                    if ((fontName.endsWith(".ttf") || fontName.endsWith(".TTF"))
                        ...
                        if ("woff".equals(format)) {
                            ...
                            data.copyTo(new FileOutputStream(absolutePath + File.separatorChar + dirname + File.separatorChar + fileName + ".woff"));
                        } else if ("ttf".equals(format)) {
                            fontFactory.serializeFont(subsetFont,
                                    new FileOutputStream(absolutePath + File.separatorChar + dirname + File.separatorChar + fileName + ".ttf"));
                        } else if ("eot".equals(format)) {

                            WritableFontData eotData = new EOTWriter(Boolean.TRUE).convert(newFont);
                            eotData.copyTo(new FileOutputStream(absolutePath + File.separatorChar + dirname + File.separatorChar + fileName + ".eot"));
                        }
    ...
    private static Font subsetFont(FontFactory fontFactory, Font font, String text, boolean stripHinting) throws IOException {
        Font newFont = font;
        if (text != null) {
