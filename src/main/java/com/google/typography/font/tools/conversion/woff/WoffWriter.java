...
public class WoffWriter {
	...
	protected boolean woff_compression_faster = true;
	...
	public WritableFontData convert(Font font) {
		...
		int length = WOFF_HEADER_SIZE + computeTableDirectoryEntriesLength(tableDirectoryEntries) + computeTablesLength(tableDirectoryEntries);
		WritableFontData writableFontData = WritableFontData.createWritableFontData(length);
		int index = 0;

		index += writeWoffHeader(writableFontData, index, tableDirectoryEntries, font.sfntVersion(), length, extractMajorVersion(font),
	...
	private int writeWoffHeader(WritableFontData writableFontData, int start, List<TableDirectoryEntry> tableDirectoryEntries, int flavor,
			int length, int majorVersion, int minorVersion) {
		int index = start;
		...
		// totalSfntSize
		index += writableFontData.writeULong(index, computeUncompressedTablesLength(tableDirectoryEntries)
	...
	private void setCompressedTableData(TableDirectoryEntry tableDirectoryEntry, Table table) {
		...
		if (woff_compression_faster && (length < 100 || table.headerTag() == Tag.loca)) {
			...
			tableDirectoryEntry
					.setCompTable(compLength == length || !compresser.finished() ? input : Arrays.copyOfRange(output, 0, compLength));
		}
	}
